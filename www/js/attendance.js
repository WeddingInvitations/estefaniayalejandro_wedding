...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var bebidaNo = document.getElementById("bebidaNo");
  var bebidaSi = document.getElementById("bebidaSi");


  checkboxSi.addEventListener("change", habilitarCampoAcompanante);
  checkboxNo.addEventListener("change", habilitarCampoAcompanante);
  bebidaNo.addEventListener("change", habilitarBebida);
  bebidaSi.addEventListener("change", habilitarBebida);
  ...
  function habilitarBebida(event) {
    // Desmarcar el otro checkbox
    if (bebidaNo.checked && event.target === bebidaSi) {
      bebidaNo.checked = false;
    } else if (bebidaSi.checked && event.target === bebidaNo) {
      bebidaSi.checked = false;
    }
  }
...
function addAcompanante() {
  ...
  nuevoParrafo.innerHTML = `
        <div class="acompanante">
          ...
          <div class="col-12 d-flex pt-3 select_edad">
            <select class="type col-sm-5 col-lg-4" name="tipo_acompanante[]">
              <option value="Adulto">Adulto</option>
              <option value="Niño">Menor</option>
            </select>
            <div class="edad-container col-sm-7 col-lg-8 mb-3"  style="display: none;">
...
document.getElementById('attendance-form').addEventListener("submit", function (event) {
  ...
  var bebida = document.getElementById("bebidaSi").checked;
...
async function enviarEmail(emailData) {
  ...
  try {
    const response = await fetch('https://us-central1-estefaniayalejandrowedding.cloudfunctions.net/enviarEmail', {
      method: 'POST',
      headers: {
